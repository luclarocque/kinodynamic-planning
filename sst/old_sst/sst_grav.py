...
XSTART = 160.0
YSTART = 120.0
XGOAL = XDIM/1.75 + random.random()*XDIM/3
YGOAL = YDIM/1.75 + random.random()*YDIM/3
...
Uy = [-19.8, 0.0]    # min - max range of y-control  (note: pixels measured from top left, so y negated)
U = [Ux, Uy]  
...
def MonteCarlo_Prop(x, U, Tprop):
    h = 0.01  # discretizing time of propagation into steps of size h
...
def main():
    ...
    while repeat:
        ...
        iternum += 1
        pygame.image.save(screen, "sstgrav{:0>2}.bmp".format(iternum))
