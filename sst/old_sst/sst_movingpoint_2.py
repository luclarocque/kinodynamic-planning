...
Uy = [-5.0, 5.0]    # min - max range of y-control  (note: pixels measured from top left, so y negated)
U = [Ux, Uy]  
...
# planning parameters
delta_bn = 15.0 # radius in which to look for nodes which are in the tree
delta_s = 50.0  # radius of each witness node (larger => sparser)
Tprop = 2.0   # max time to propagate with one control input
goalbias = 0.01  # percentage of time sample is taken to be in goal region
epsilon = 0.96  # factor by which we reduce delta_bn/s with each loop (for SST*)
...
#  and the new cost 
def MonteCarlo_Prop(x, U, Tprop, h=0.01):
...
def prune_nodes(newnode, snew, Vac, Vinac, E):
    ...
    while is_leaf(xpeer) and xpeer in Vinac:
        #print("xpeer is leaf in Vinac: REMOVING")
...
def main():
    ...
    while repeat:
        ...
        for i in xrange(N):
            ...
            #print("Vac: {}, Vinac: {}".format(len(Vac), len(Vinac)))
