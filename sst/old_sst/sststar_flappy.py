...
def main():
    ...
    while repeat:
        ...
        iternum += 1
        pygame.image.save(screen, "sststar-flappy{:0>2}.bmp".format(iternum))

        # SST* additional updates
        delta_s = zeta * delta_s
        delta_bn = zeta * delta_bn
        j = iternum
        #N = int( (1 + log(j)) * zeta**(-j*(len(xnew.state) + len(u) + 1)) * N )
        N = int( N * (1 + log(j)) * zeta**(-log(j)*(2)) )
