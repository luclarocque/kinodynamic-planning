...
# trueflappy uses only positive values for the x-control, and the y-control is either
#   the maximum value or zero, allowing for upward acceleration "on" or "off" for the
#   duration of that particular propagation.
...
YGOAL = YDIM/3 + random.random()*YDIM/3
...
# control space
Ux = [3.0, 3.0]   # min - max range of x-control  
...
OBS=[(XDIM/3, 0, 40, YDIM/2),\
     ...
     (XDIM*5/7, 0, 40, YDIM*4/7),\
     (XDIM*5/7, YDIM*5/7, 40, YDIM*2/7)]
...
def main():
    ...
    while repeat:
        ...
        for i in xrange(N):
            V = Vac + Vinac
            xselected = best_first_selection(V, delta_bn, goalbias) 
        ...
        # iternum += 1
        # if iternum == 1 or iternum % 4 == 0:
        #     pygame.image.save(screen, "sst-trueflappy{:0>2}.bmp".format(iternum))
